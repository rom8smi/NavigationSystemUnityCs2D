...
namespace GenericCode
{
    public class KDTree2D
    {
        ...
        public static KDTree2D MakeFromPoints(params Float2[] points)
        {
        ...
        static int FindSplitPoint(Float2[] points, int[] inds, int stIndex, int enIndex, int axis)
        {
            float a = points[inds[stIndex]].GetAxis(axis);
            float b = points[inds[enIndex]].GetAxis(axis);
            int midIndex = (stIndex + enIndex) / 2;
            float m = points[inds[midIndex]].GetAxis(axis);
        ...
        //	of its plane.
        public static int FindPivotIndex(Float2[] points, int[] inds, int stIndex, int enIndex, int axis)
        {
            int splitPoint = FindSplitPoint(points, inds, stIndex, enIndex, axis);

            Float2 pivot = points[inds[splitPoint]];
            ...
            while (currPt <= endPt)
            {
                Float2 curr = points[inds[currPt]];

                if (curr.GetAxis(axis) > pivot.GetAxis(axis))
                {
        ...
        void Search(Float2 pt, ref float bestSqSoFar, ref int bestIndex)
        {
            ...
            float planeDist = pt.GetAxis(axis) - pivot.GetAxis(axis);
        ...
        public void FindNearestsBall(Float2 pt, float r, List<int> neighburs)
        {
            float bestSqDist = float.MaxValue;
            int bestIndex = -1;
            float rSq = r * r;
            SearchBall(pt, ref bestSqDist, ref bestIndex, ref rSq, neighburs);
        }

        public void FindNearestsBallSort(Float2 pt, float r, List<int> neighburs)
        {
            float bestSqDist = float.MaxValue;
            int bestIndex = -1;
            float rSq = r * r;
            List<float> sqDistances = new List<float>();
            SearchBall(pt, ref bestSqDist, ref bestIndex, ref rSq, sqDistances, neighburs);
            HeapSort.Sort(neighburs, sqDistances, neighburs.Count);
        }

        void SearchBall(Float2 pt, ref float bestSqSoFar, ref int bestIndex, ref float radiusSqr, List<int> neighbours)
        {
            float mySqDist = (pivot - pt).LengthSquared();
            ...
            if (mySqDist < bestSqSoFar)
            {
                ...
                if (mySqDist <= radiusSqr)
                {
                    neighbours.Add(bestIndex);
                    bestSqSoFar = radiusSqr;
                }
            }
            ...
            if (lr[selector] != null)
            {
                lr[selector].SearchBall(pt, ref bestSqSoFar, ref bestIndex, ref radiusSqr, neighbours);
            }
        ...
        void SearchBall(Float2 pt, ref float bestSqSoFar, ref int bestIndex, ref float radiusSqr, List<float> sqDistances, List<int> neighbours)
        {
            float mySqDist = (pivot - pt).LengthSquared();
            ...
            if (lr[selector] != null)
            {
                lr[selector].SearchBall(pt, ref bestSqSoFar, ref bestIndex, ref radiusSqr, sqDistances, neighbours);
            }
        ...
        public int FindNearestWithMaxIndex(Float2 pt, int maxIndex)
        {
            float bestSqDist = float.MaxValue;
            int bestIndex = -1;

            SearchWithMaxIndex(pt, ref bestSqDist, ref maxIndex, ref bestIndex);

            return bestIndex;
        }

        void SearchWithMaxIndex(Float2 pt, ref float bestSqSoFar, ref int maxIndex, ref int bestIndex)
        {
            ...
            if (lr[selector] != null)
            {
                lr[selector].SearchWithMaxIndex(pt, ref bestSqSoFar, ref maxIndex, ref bestIndex);
            }
